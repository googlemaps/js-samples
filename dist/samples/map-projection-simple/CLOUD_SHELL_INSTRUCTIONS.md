# Run the `map-projection-simple` sample in Google Cloud Shell

<walkthrough-tutorial-duration duration="10"/>

## Introduction

This tutorial will walk through the process of running a web application from the `map-projection-simple` sample using TypeScript, Babel, and Webpack.
...
```bash
googlemaps-js-samples init -v --no-open --no-hot map-projection-simple .
```
